...
initial_v_inf = 14; % km/s
sequence = [2, 7]; % 1 = Venus --> 7 = Neptune
max_lengths = [8000]; % transfer times (days)
...
for time_elapsed = 0:1:0 % adjust to test multiple launch days
    r_out = zeros(1, 3);
    ...
    if ~failed
        ...
        disp(strcat("Total ΔV: ", num2str(total_dv), " km/s (M1: ", num2str(dv_launch),...
            " km/s, M2: ", num2str(dv_capture), " km/s)"))
